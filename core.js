...
function kill(){
  ...
  for(const s of SK)if(s.unlock>prevBest&&s.unlock<=S.best){banner('새 스킬 해금',s.name,s.c,2.2);sfx.chime();if(S.equip.length<7&&!S.equip.includes(s.id))S.equip.push(s.id);buildBar();buildBook()}
  stageUI();
}

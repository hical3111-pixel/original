...
function labSilence(){if(master&&AC){master.disconnect();master=AC.createGain();master.gain.value=.32;master.connect(AC.destination)}for(const k in thr)delete thr[k]}
...
function labClear(){
  labSilence();FX.length=P.length=T.length=C.length=B.length=PR.length=CR.length=relicQ.length=0;
...
function labPlay(kind,id){
  ...
  $('labStatus').textContent=item.name+' · 재생 중';
...
function labTick(dt){
  ...
  if(!lab.playing&&lab.repeat&&lab.selected){lab.gap-=dt;if(lab.gap<=0)labPlay(lab.selected.kind,lab.selected.id)}
}
...
function labStart(){
  ...
  S.auto=false;S.sound=raw.sound!==false;S.loadout=[];S.equip=[];ST=stats(S.lv,false);lab.tier=S.lv.skill>=20?3:S.lv.skill>=10?2:S.lv.skill>=5?1:0;
